...
//định nghĩa các rule
//khi có lỗi trả ra messentger lỗi 
//khi hợp lệ thì không tra ra gì hết (undefined)
...
Validator.isEmail =function(selector,mes) {
     return {
         ...
         test: function(value){
            ...
            return regex.test(value)? undefined : mes || "Vui lòng nhập đúng Email"
         }
     }
}
Validator.minLenth =function(selector ,min ,mes){
    return {
        selector: selector,
        test:function(value){
            return value.length >=min? undefined : mes || `Vui lòng nhập tối thiểu ${min} kí tự`
        }
    }
}


Validator.isComfirmed = function(selector ,getConfirvalue ,mes) {
    return{
        selector:selector,
        test:function(value){
            return value === getConfirvalue() ? undefined : mes || "Giá trị nhập vào không chính xác"
        }
    }
